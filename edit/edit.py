    ...
    for section in config.sections():
        if section.lower().startswith("page"):
            ...
            for btn_id in sorted(btn_ids):
                ...
                while True:
                    ...
                    if ctype == "close_all_windows":
                        ...
                        if len(ctype_parts) >= 3:
                            ...
                            state = ctype_parts[2].strip()
                        elif len(ctype_parts) == 2:
                            udp_command_id = ctype_parts[1].strip()
                            state = ""
                        else:
                            continue  # 格式错误
                ...
                try:
                    switch_port = int(config.get(section, f"{btn_id}.switch_port", fallback="5000"))
                except ValueError:
                    switch_port = 5000
    ...
    for page in data['pages']:
        ...
        for btn in page.get('buttons', []):
            ...
            # 保存开关按钮的on_src和off_src属性
                ...
                # 保存编码格式
                config[sec][f"{prefix}.encoding"] = btn.get('encoding', '16进制')
            # 保存空调面板的特有属性
            ...
            if btn.get('status_enable', False):
                ...
                config[sec][f"{prefix}.status_encoding"] = btn.get('status_encoding', '16进制')
        ...
        for text in page.get('texts', []):
            ...
            h = int(text['h'])
            config[sec][f"{prefix}.pos"] = f"{x},{y},{w},{h}"
            config[sec][f"{prefix}.text_content"] = text.get('text', '')
            config[sec][f"{prefix}.font_family"] = text.get('font_family', 'Microsoft YaHei')
            config[sec][f"{prefix}.color"] = text.get('color', '#000000')
            config[sec][f"{prefix}.align"] = text.get('align', 'left')
            config[sec][f"{prefix}.bold"] = str(text.get('bold', False))
            config[sec][f"{prefix}.italic"] = str(text.get('italic', False))
...
class TextPropsDlg(QDialog):
    ...
    def __init__(self, cfg, logic_w, logic_h):
        ...
        self.logic_w = logic_w
        self.logic_h = logic_h
        self.setWindowTitle("文字属性")
        
        lay = QFormLayout(self)
        
        # 解析ID和名称
        text_id = self.cfg.get('id', '')
        if '_' in text_id:
            id_parts = text_id.split('_', 1)
            text_id = id_parts[0]
            default_name = id_parts[1] if len(id_parts) > 1 else f"文字{text_id}"
        else:
            default_name = self.cfg.get('name', f"文字{text_id}")
        
        # ID 和名称行
        id_lay = QHBoxLayout()
        id_lay.addWidget(QLabel("ID"))
        self.id_edit = QLineEdit(text_id)
        self.id_edit.setValidator(QRegularExpressionValidator(QRegularExpression(r'[a-zA-Z0-9_]+')))
        id_lay.addWidget(self.id_edit)
        ...
        # 文字内容
        self.text_edit = QLineEdit(self.cfg.get('text', '文字'))
        lay.addRow("文字内容", self.text_edit)
        
        # 位置和大小
        w = QSpinBox()
        w.setRange(10, self.logic_w)
        w.setValue(int(self.cfg.get('w', 200)))
        h = QSpinBox()
        h.setRange(10, self.logic_h)
        h.setValue(int(self.cfg.get('h', 50)))
        x = QSpinBox()
        x.setRange(0, self.logic_w)
        x.setValue(int(self.cfg.get('x', 0)))
        y = QSpinBox()
        y.setRange(0, self.logic_h)
        y.setValue(int(self.cfg.get('y', 0)))
        
        wh_lay = QHBoxLayout()
        wh_lay.addWidget(QLabel("宽"))
        wh_lay.addWidget(w)
        wh_lay.addWidget(QLabel("高"))
        wh_lay.addWidget(h)
        lay.addRow("尺寸", wh_lay)
        ...
        self.color_edit = QLineEdit(self.cfg.get('color', '#000000'))
        self.color_btn = QPushButton("选择颜色")
        self.color_btn.clicked.connect(self.choose_color)
        color_lay.addWidget(self.color_edit)
        color_lay.addWidget(self.color_btn)
        lay.addRow("字体颜色", color_lay)
        
        # 字体选择
        self.font_combo = QFontComboBox()
        current_font = self.cfg.get('font_family', 'Microsoft YaHei')
        self.font_combo.setCurrentFont(QFont(current_font))
        lay.addRow("字体", self.font_combo)
        
        # 对齐方式
        self.align_combo = QComboBox()
        self.align_combo.addItems(["左对齐", "居中", "右对齐"])
        align_map = {'left': 0, 'center': 1, 'right': 2}
        self.align_combo.setCurrentIndex(align_map.get(self.cfg.get('align', 'left'), 0))
        lay.addRow("对齐方式", self.align_combo)
        
        # 粗体和斜体
        style_lay = QHBoxLayout()
        self.bold_check = QCheckBox("粗体")
        self.bold_check.setChecked(self.cfg.get('bold', False))
        self.italic_check = QCheckBox("斜体")
        self.italic_check.setChecked(self.cfg.get('italic', False))
        style_lay.addWidget(self.bold_check)
        style_lay.addWidget(self.italic_check)
        lay.addRow("字体样式", style_lay)
    ...
    def choose_color(self):
        """选择颜色"""
        color = QColorDialog.getColor(QColor(self.color_edit.text()), self, "选择文字颜色")
        if color.isValid():
            self.color_edit.setText(color.name())
    
    def accept(self):
        text_id = self.id_edit.text().strip()
        if not text_id:
            QMessageBox.warning(self, "错误", "ID不能为空")
            return
        
        text_name = self.name_edit.text().strip() or f"文字{text_id}"
...
class SwitchPropsDlg(QDialog):
    def __init__(self, cfg, logic_w, logic_h):
        ...
        # IP 和端口设置
        print("DEBUG: 正在创建 IP 和端口设置")
        switch_ip_layout = QHBoxLayout()
        ip_label = QLabel("IP 地址:")
        ip_label.setStyleSheet("color: red; font-weight: bold;")
        switch_ip_layout.addWidget(ip_label)
        self.switch_ip_edit = QLineEdit(self.cfg.get('switch_ip', ''))
        self.switch_ip_edit.setStyleSheet("background-color: yellow;")
        switch_ip_layout.addWidget(self.switch_ip_edit)
        
        switch_port_layout = QHBoxLayout()
        switch_port_layout.addWidget(QLabel("端口号:"))
        self.switch_port_spin = QSpinBox()
        self.switch_port_spin.setRange(1, 65535)
        self.switch_port_spin.setValue(self.cfg.get('switch_port', 5000))
        switch_port_layout.addWidget(self.switch_port_spin)
        
        on_cmd_layout.addLayout(switch_ip_layout)
        on_cmd_layout.addLayout(switch_port_layout)
        ...
        # 编码格式选择
        udp_query_layout.addWidget(QLabel("编码:"), 0, 4)
        status_encoding_combo = QComboBox()
        status_encoding_combo.addItems(["16进制", "字符串"])
        saved_encoding = self.cfg.get('status_encoding', '16进制')
        status_encoding_combo.setCurrentText(saved_encoding)
        udp_query_layout.addWidget(status_encoding_combo, 0, 5)
        ...
        self.status_encoding_combo = status_encoding_combo
    ...
    def accept(self):
        ...
        self.cfg_out = {
            ...
            'device_cmd_index': device_cmd_index,
            # 开关 IP 和端口设置
            'switch_ip': self.switch_ip_edit.text(),
            'switch_port': self.switch_port_spin.value(),
            # 状态显示设置
            ...
            'status_encoding': self.status_encoding_combo.currentText(),
    ...
    def _load_device_commands(self, device_id):
        ...
        for device in devices:
            if device.get('id') == device_id:
                commands = device.get('commands', [])
                print(f"device_id: {device_id}, commands count: {len(commands)}")
                # 添加指令到下拉框
        ...
        print(f"cmd_index: {cmd_index}, count: {self.cmd_combo.count()}")
        if cmd_index > 0 and cmd_index <= len(commands):
            cmd_name = commands[cmd_index - 1].get('name', f"指令{cmd_index}")
            self.cmd_combo.setCurrentText(cmd_name)
            print(f"set current text to {cmd_name}")
            # 延迟设置显示项
            from PySide6.QtCore import QTimer
            QTimer.singleShot(0, lambda: self.selected_cmd_label.setText(f"已选择: {cmd_name}"))
            QTimer.singleShot(0, lambda: self.selected_cmd_label.setStyleSheet("color: #000"))
            # 强制处理事件
            ...
            # 延迟设置
            from PySide6.QtCore import QTimer
            QTimer.singleShot(0, lambda: self.selected_cmd_label.setText("未选择指令"))
            QTimer.singleShot(0, lambda: self.selected_cmd_label.setStyleSheet("color: #666"))
        
        # 强制更新UI
        self.cmd_combo.update()
        self.cmd_combo.repaint()
        self.selected_cmd_label.update()
        self.selected_cmd_label.repaint()
...
class SwitchPropsDlg(QDialog):
    def __init__(self, cfg, logic_w, logic_h):
        ...
        cmd_select_layout.addWidget(QLabel("选择指令:"))
        self.cmd_combo = QComboBox()
        ...
        cmd_select_layout.addWidget(self.selected_cmd_label)
        cmd_select_layout.addStretch()
        ...
        self.switch_ip_edit = QLineEdit(self.cfg.get('switch_ip', ''))
        ip_layout.addWidget(self.switch_ip_edit)
        cmd_layout.addLayout(ip_layout)
        ...
        self.switch_port_spin.setRange(1, 65535)
        self.switch_port_spin.setValue(self.cfg.get('switch_port', 5000))
        port_layout.addWidget(self.switch_port_spin)
        
        # 编码格式选择
        port_layout.addWidget(QLabel("编码:"))
        self.encoding_combo = QComboBox()
        self.encoding_combo.addItems(["16进制", "字符串"])
        # 读取保存的编码格式，默认为16进制
        saved_encoding = self.cfg.get('encoding', '16进制')
        self.encoding_combo.setCurrentText(saved_encoding)
        port_layout.addWidget(self.encoding_combo)
        
        cmd_layout.addLayout(port_layout)
        ...
        self.cmd_combo.currentIndexChanged.connect(self.on_cmd_selection_changed)
    ...
    def on_cmd_selection_changed(self, index):
        """指令选择改变时的处理"""
        if index <= 0:
            self.selected_cmd_label.setText("未选择指令")
            self.selected_cmd_label.setStyleSheet("color: #666")
        else:
            cmd = self.cmd_combo.itemData(index)
            if cmd:
                cmd_name = cmd.get('name', "未知指令")
                self.selected_cmd_label.setText(f"已选择: {cmd_name}")
    ...
    def on_device_check_changed(self, state):
        ...
        print(f"on_device_check_changed called: state={state}, enabled={enabled}")
        
        # 立即设置下拉框的启用状态（无论勾选还是取消）
        self.device_combo.setEnabled(enabled)
        self.cmd_combo.setEnabled(enabled)
        print(f"device_combo enabled: {self.device_combo.isEnabled()}")
        print(f"cmd_combo enabled: {self.cmd_combo.isEnabled()}")
        
        # 当勾选设备时，禁用手动指令输入和IP端口输入
        ...
        self.switch_ip_edit.setEnabled(not enabled)
        self.switch_port_spin.setEnabled(not enabled)
        
        # 强制刷新UI
        self.device_combo.repaint()
        self.cmd_combo.repaint()
        ...
        if enabled:
            ...
            # 更新指令列表（根据当前选中的设备）
            # 注意：如果device_id为空，设备索引为0，on_device_changed会清空指令列表
            self.on_device_changed(self.device_combo.currentIndex())
            
            # 如果找到设备且有保存的指令索引，设置指令选择
            if device_found and cmd_index >= 1:
                # cmd_index是1-based，下拉框也是1-based（第0项是"请选择指令"）
                if cmd_index < self.cmd_combo.count():
                    self.cmd_combo.setCurrentIndex(cmd_index)
                    # 更新显示的标签
                    cmd = self.cmd_combo.itemData(cmd_index)
                    if cmd:
                        cmd_name = cmd.get('name', f"指令{cmd_index}")
                        self.selected_cmd_label.setText(f"已选择: {cmd_name}")
                        self.selected_cmd_label.setStyleSheet("color: #000")
            elif not device_found:
                # 如果没有找到设备，确保指令下拉框显示默认状态
                self.selected_cmd_label.setText("未选择指令")
                self.selected_cmd_label.setStyleSheet("color: #666")
    ...
    def on_device_changed(self, index):
        ...
        device_port = device.get('port', 5000)
        self.switch_ip_edit.setText(device_ip)
        self.switch_port_spin.setValue(device_port)
    ...
    def _load_saved_settings(self):
        ...
        # 直接设置下拉框的启用状态
        self.device_combo.setEnabled(device_use)
        self.cmd_combo.setEnabled(device_use)
        self.on_cmd_edit.setEnabled(not device_use)
        self.off_cmd_edit.setEnabled(not device_use)
        self.query_cmd_edit.setEnabled(not device_use)
        self.response_cmd_edit.setEnabled(not device_use)
        self.switch_ip_edit.setEnabled(not device_use)
        self.switch_port_spin.setEnabled(not device_use)
        
        if device_use:
            # 只要启用了设备，就加载设备列表
            self.load_devices()
            
        if device_use and device_id:
            # 查找并设置设备
            device_found = False
            for i in range(self.device_combo.count()):
                device = self.device_combo.itemData(i)
            ...
            if device_found:
                # 获取选中的设备
                device = self.device_combo.itemData(self.device_combo.currentIndex())
                if device:
                    # 加载指令列表
                    self.cmd_combo.clear()
                    self.cmd_combo.addItem("请选择指令")
                    ...
                    # 查找并设置指令（cmd_index是1-based，1=第1路指令）
                    # 下拉框第0项是"请选择指令"，实际指令从第1项开始
                    if cmd_index >= 1 and cmd_index <= len(commands):
                        combo_index = cmd_index  # 直接使用cmd_index（因为下拉框索引也是1-based）
                        if combo_index < self.cmd_combo.count():
                            self.cmd_combo.setCurrentIndex(combo_index)
                            # 更新显示的标签
                            cmd = self.cmd_combo.itemData(combo_index)
                            if cmd:
                                cmd_name = cmd.get('name', f"指令{cmd_index}")
                                self.selected_cmd_label.setText(f"已选择: {cmd_name}")
                                self.selected_cmd_label.setStyleSheet("color: #000")
    ...
    def accept(self):
        ...
        if device_use:
            ...
            # 获取选中的指令（下拉框索引是1-based，因为第0项是"请选择指令"）
            # 直接保存下拉框索引（1=第1路，2=第2路）
            cmd_index = self.cmd_combo.currentIndex()
            if cmd_index > 0:
                device_cmd_index = cmd_index  # 直接保存（1-based）
    ...
    def _load_saved_command(self):
        ...
        for cmd in commands:
            if cmd.get('type') == 'udp' and 'udp_command_id' in cmd:
                ...
                # 从指令表中获取命令名称
                self.selected_cmd_name = self.selected_cmd_id
                for udp_cmd in udp_commands:
                    if udp_cmd.get('id') == self.selected_cmd_id:
                        self.selected_cmd_name = udp_cmd.get('name', self.selected_cmd_id)
                        break
                self.selected_cmd_label.setText(f"已选择: {self.selected_cmd_name}")
                self.selected_cmd_label.setStyleSheet("color: #000")
                break
                ...
                self.selected_cmd_name = self.selected_cmd_id
                for group in udp_groups:
                    if group.get('id') == self.selected_cmd_id:
                        self.selected_cmd_name = group.get('name', self.selected_cmd_id)
                        break
                self.selected_cmd_label.setText(f"已选择: {self.selected_cmd_name}")
                self.selected_cmd_label.setStyleSheet("color: #000")
                break
...
class TextItem(QGraphicsTextItem):
    ...
    def __init__(self, cfg, logic_w, logic_h, parent=None):
        ...
        if not self.cfg.get('id'):
            ...
            if hasattr(self, 'scene') and hasattr(self.scene(), 'page_cfg'):
                existing_ids = {item.get('id', '') for item in self.scene().page_cfg.get('texts', [])}
        ...
        if 'name' not in self.cfg:
            ...
            self.cfg['name'] = f"文字{num}"
        
        # 设置默认配置
        self.cfg.setdefault('type', 'text')
        self.cfg.setdefault('text', '文字')
        self.cfg.setdefault('x', 0)
        self.cfg.setdefault('y', 0)
        self.cfg.setdefault('w', 200)
        self.cfg.setdefault('h', 50)
        self.cfg.setdefault('font_family', 'Microsoft YaHei')
        self.cfg.setdefault('color', '#000000')
        self.cfg.setdefault('align', 'left')
        self.cfg.setdefault('bold', False)
        self.cfg.setdefault('italic', False)
        
        self.update_appearance()
        self.setPos(self.cfg['x'], self.cfg['y'])
    ...
    def update_appearance(self):
        """更新文字外观"""
        # 计算字体大小，使文字能撑满整个控件
        text = self.cfg.get('text', '文字')
        font_family = self.cfg.get('font_family', 'Microsoft YaHei')
        ...
        while low <= high:
            ...
            test_font.setBold(self.cfg.get('bold', False))
            test_font.setItalic(self.cfg.get('italic', False))
            
            # 使用 QFontMetrics 计算文字尺寸
            metrics = QFontMetrics(test_font)
            text_width = metrics.horizontalAdvance(text)
            text_height = metrics.height()
            
            # 检查是否适合控件大小（留一些边距）
            if text_width <= target_w - 4 and text_height <= target_h - 4:
                best_size = mid
                low = mid + 1
            else:
                high = mid - 1
        ...
        font.setBold(self.cfg.get('bold', False))
        font.setItalic(self.cfg.get('italic', False))
        self.setFont(font)
        ...
        # 设置颜色
        color = QColor(self.cfg.get('color', '#000000'))
        ...
        self.setTextWidth(self.cfg['w'])
        
        # 设置文本格式
        doc = self.document()
        block_format = QTextBlockFormat()
        block_format.setAlignment(align_map.get(self.cfg.get('align', 'left'), Qt.AlignLeft))
    ...
    def paint(self, painter, option, widget=None):
        ...
        # 绘制背景（仅选中时）
        if self.isSelected():
            painter.fillRect(self.boundingRect(), QColor(200, 200, 255, 100))
    ...
    def boundingRect(self):
        """返回边界矩形"""
        return QRectF(0, 0, self.cfg['w'], self.cfg['h'])
    ...
    def mousePressEvent(self, event):
        ...
        if hasattr(self, '_scene') and self._scene:
            self._scene.on_text_clicked(self, event)
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.resizing:
            ...
            new_w = max(10, self.resize_start_rect.width() + delta.x())
            new_h = max(10, self.resize_start_rect.height() + delta.y())
            self.cfg['w'] = int(new_w)
            self.cfg['h'] = int(new_h)
            self.update_appearance()
            self.update()
            event.accept()  # 接受事件，阻止继续传播
            return
    ...
    def mouseReleaseEvent(self, event):
        ...
        # 更新位置到配置
        pos = self.pos()
        self.cfg['x'] = int(pos.x())
        self.cfg['y'] = int(pos.y())
    ...
    def edit_props(self):
        """编辑属性"""
        dlg = TextPropsDlg(self.cfg, self.logic_w, self.logic_h)
        if dlg.exec():
    ...
    def duplicate(self):
        ...
        # 生成唯一ID
        existing_ids = {item['id'] for item in self.scene().page_cfg.get('texts', [])}
        ...
        new_cfg['id'] = new_id
        new_cfg['x'] = self.cfg['x'] + 20
        new_cfg['y'] = self.cfg['y'] + 20
        
        self.scene().page_cfg.setdefault('texts', []).append(new_cfg)
        self.scene().add_text(new_cfg)
        self.scene().update()
    ...
    def delete(self):
        ...
        if scene:
            # 从配置中移除
            texts = scene.page_cfg.get('texts', [])
            for i, text in enumerate(texts):
                if text['id'] == self.cfg['id']:
                    texts.pop(i)
                    break
            # 从场景中移除
...
class Scene(QGraphicsScene):
    ...
    def load_texts(self):
        ...
        def get_text_number(text):
            import re
            match = re.search(r'\d+', text['id'])
            return int(match.group()) if match else 0
        
        texts = sorted(self.page_cfg.get('texts', []), key=get_text_number)
        for text_cfg in texts:
            self.add_text(text_cfg)
    ...
    def add_text(self, cfg):
        """添加文字项"""
        item = TextItem(cfg, self.logic_w, self.logic_h)
        self.addItem(item)
        item.setFlag(QGraphicsItem.ItemIsSelectable, True)
        item.setAcceptedMouseButtons(Qt.LeftButton)
    ...
    def on_text_clicked(self, text_item, event):
        """文字项被点击"""
        # 清除所有其他项的选中状态
        for item in self.items():
            if isinstance(item, (ButtonItem, TextItem)) and item != text_item:
...
class Editor(QMainWindow):
    def __init__(self):
        ...
        add_widget_menu.addAction("添加文字", self.add_text)
                ...
                if scene:
                    ...
                    for item in scene.items():
                        if isinstance(item, ButtonItem):
                            ...
                            text_list.append(item.cfg)
                    self.cfg['pages'][self.last_current_row]['buttons'] = btn_list
                    self.cfg['pages'][self.last_current_row]['texts'] = text_list
    ...
    def add_text(self):
        ...
        # 生成唯一ID和名称
        existing_ids = {item['id'].split('_')[0] for item in page.get('texts', [])}
        idx = 1
        while f"text{idx}" in existing_ids:
            idx += 1
        text_name = f"文字{idx}"
        text_id = f"text{idx}_{text_name}"  # 使用下划线分隔ID和名称

        cfg = {
            "id": text_id,  # ID格式: text1_文字1
            "x": 100,
            "y": 100,
            "w": 200,
            "h": 50,
        ...
        page.setdefault('texts', []).append(cfg)
        self.view.scene().add_text(cfg)
        self.view.scene().update()
